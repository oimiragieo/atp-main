...
class CacheManager:
            ...
            try:
                ...
                try:
                    ...
                    item = await asyncio.wait_for(
                        self._invalidation_queue.get(), timeout=self.config.invalidation_timeout
                    )
                    batch.append(item)
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        health = {"l1_cache": True, "l2_cache": True, "overall": True}
...
async def initialize_cache_manager(config: CacheConfig | None = None) -> CacheManager:
