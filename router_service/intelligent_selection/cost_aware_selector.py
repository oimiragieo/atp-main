...
from .selection_config import SelectionConfig, LOCAL_MODEL_INDICATORS, LOCAL_PROVIDERS
...
class CostAwareSelector:
    ...
    async def select_optimal_model(
        ...
        try:
            ...
            enhanced_candidates = await self._enhance_candidates_with_pricing(
                candidates, estimated_tokens
            )
            ...
            selection_metadata.update({
                "error": str(e),
                "fallback_used": True,
                "selected_models": [c.name for c in fallback_candidates]
            })
    ...
    async def _cost_aware_bandit_selection(
        ...
        if scored_candidates:
            ...
            import random
            if (len(scored_candidates) > 1 and 
    ...
    async def _get_performance_multiplier(self, model_name: str) -> float:
        ...
        multiplier = (
            success_rate * 0.4 +
            avg_quality * 0.4 +
            (1.0 / max(avg_latency_ratio, 0.1)) * 0.2
        )
        
        return max(0.5, min(1.5, multiplier))
    
    def _pure_cost_selection(
        self,
        candidates: List[Candidate],
        latency_slo_ms: int
    ) -> List[Candidate]:
