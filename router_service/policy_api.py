...
class PolicyRuleModel(BaseModel):
    ...
    resources: list[str] | None = Field(None, description="Resource patterns (supports wildcards)")
    actions: list[str] | None = Field(None, description="Applicable actions")
...
class PolicyTestRequest(BaseModel):
    ...
    user_id: str | None = Field(None, description="User ID for testing")
    tenant_id: str | None = Field(None, description="Tenant ID for testing")
...
class PolicyTestResponse(BaseModel):
    ...
    effect: Effect | None = Field(None, description="Final policy effect")
...
async def create_policy(policy: ABACPolicyModel, user: UserInfo = Depends(require_admin())) -> ABACPolicyModel:
...
async def list_policies(
    ...
    return PolicyListResponse(policies=api_policies, total=len(all_policies), page=page, page_size=page_size)


@policy_router.get("/{policy_id}", response_model=ABACPolicyModel)
async def get_policy(
    policy_id: str, user: UserInfo = Depends(require_authentication({"read", "write", "admin"}))
) -> ABACPolicyModel:
...
async def update_policy(
    policy_id: str, policy: ABACPolicyModel, user: UserInfo = Depends(require_admin())
) -> ABACPolicyModel:
...
async def delete_policy(policy_id: str, user: UserInfo = Depends(require_admin())) -> dict[str, str]:
...
async def enable_policy(policy_id: str, user: UserInfo = Depends(require_admin())) -> dict[str, str]:
...
async def disable_policy(policy_id: str, user: UserInfo = Depends(require_admin())) -> dict[str, str]:
...
async def test_policies(
    request: PolicyTestRequest, user: UserInfo = Depends(require_authentication({"read", "write", "admin"}))
) -> PolicyTestResponse:
...
async def validate_policy(
    policy: ABACPolicyModel, user: UserInfo = Depends(require_authentication({"read", "write", "admin"}))
) -> dict[str, Any]:
    ...
    try:
        ...
        return {"valid": True, "message": "Policy is valid", "warnings": []}

    except Exception as e:
        return {"valid": False, "message": f"Policy validation failed: {str(e)}", "errors": [str(e)]}
    ...
    for rule_model in api_policy.rules:
        ...
        for cond_model in rule_model.conditions:
            condition = AttributeCondition(
                attribute=cond_model.attribute, operator=cond_model.operator, value=cond_model.value
            )
    ...
    for rule in internal_policy.rules:
        ...
        for condition in rule.conditions:
            cond_model = AttributeConditionModel(
                attribute=condition.attribute, operator=condition.operator, value=condition.value
            )
