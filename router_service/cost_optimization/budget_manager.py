...
class BudgetManager:
    ...
    async def record_spending(
        self, cost_usd: float, tenant_id: str | None = None, project_id: str | None = None
    ) -> dict[str, Any]:
    ...
    async def _check_budget_limits(self, tenant_id: str | None = None, project_id: str | None = None) -> dict[str, Any]:
        """Check if spending is within budget limits."""
        status = {"within_budget": True, "warnings": [], "critical_alerts": [], "enforcement_actions": []}
    ...
    async def _apply_enforcement_action(
        self, entity_id: str, entity_type: str, usage_percent: float
    ) -> dict[str, Any] | None:
    ...
    async def check_request_allowed(
        self, tenant_id: str | None = None, project_id: str | None = None, estimated_cost: float | None = None
    ) -> dict[str, Any]:
        """Check if a request is allowed based on budget and rate limits."""
        current_time = time.time()

        result = {"allowed": True, "reasons": [], "throttle_factor": 1.0, "estimated_cost": estimated_cost}
        ...
        if estimated_cost and result["allowed"]:
            budget_check = await self._check_estimated_cost_impact(estimated_cost, tenant_id, project_id)
    ...
    async def _check_rate_limit(self, tenant_id: str, rate_limit: int, current_time: float) -> bool:
        ...
        self._tenant_request_counts[tenant_id] = [ts for ts in self._tenant_request_counts[tenant_id] if ts > hour_ago]
    ...
    async def _check_estimated_cost_impact(
        self, estimated_cost: float, tenant_id: str | None = None, project_id: str | None = None
    ) -> dict[str, Any]:
    ...
    async def get_budget_status(self, tenant_id: str | None = None, project_id: str | None = None) -> dict[str, Any]:
        """Get current budget status."""
        status = {"global_spending": self._global_spending, "tenant_status": {}, "project_status": {}}
