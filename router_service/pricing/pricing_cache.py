...
class PricingCache:
    ...
    async def get_pricing(self, provider: str, model: str) -> dict[str, float] | None:
    ...
    async def set_pricing(
        self, provider: str, model: str, pricing: dict[str, float], metadata: dict[str, Any] | None = None
    ) -> bool:
    ...
    def _detect_changes(
        self, previous_pricing: dict[str, float], current_pricing: dict[str, float]
    ) -> list[dict[str, Any]]:
    ...
    async def _update_pricing_history(
        self, provider: str, model: str, pricing: dict[str, float], changes: list[dict[str, Any]]
    ) -> None:
        ...
        try:
            ...
            # Add new entry
            history_entry = {"timestamp": time.time(), "pricing": pricing, "changes": changes}
    ...
    async def get_pricing_history(self, provider: str, model: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    async def get_pricing_changes(
        self, provider: str | None = None, model: str | None = None, since_timestamp: float | None = None
    ) -> list[dict[str, Any]]:
            ...
            for key in keys:
                try:
                    ...
                    if cached_data and cached_data.get("changes_detected"):
                        ...
                        if since_timestamp:
                            item_changes = [
                                change for change in item_changes if change.get("detected_at", 0) >= since_timestamp
                            ]
    ...
    async def clear_pricing_cache(self, provider: str | None = None, model: str | None = None) -> int:
        """Clear pricing cache with optional filtering."""
        try:
            if provider and model:
                # Clear specific model
                keys_to_clear = [self._make_key(provider, model), self._make_history_key(provider, model)]
                cleared_count = 0
