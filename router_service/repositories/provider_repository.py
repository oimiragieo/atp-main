...
class ProviderRepository(BaseRepository[Provider]):
    ...
    async def get_with_models(self, provider_id: UUID) -> Provider | None:
    ...
    async def get_providers_with_model_count(self) -> list[dict[str, Any]]:
        ...
        async with self.db_manager.get_session() as session:
            stmt = (
                select(Provider, func.count(Model.id).label("model_count"))
                .outerjoin(Model, and_(Model.provider_id == Provider.id, not Model.is_deleted))
                .where(not Provider.is_deleted)
            ...
            return [{"provider": row.Provider, "model_count": row.model_count} for row in rows]

    async def update_health_status(
        self, provider_id: UUID, health_status: str, last_health_check: datetime | None = None
    ) -> bool:
        """Update provider health status."""
        updates = {"health_status": health_status, "last_health_check": last_health_check or datetime.utcnow()}
    ...
    async def get_stale_health_checks(self, hours: int = 1) -> list[Provider]:
        ...
        async with self.db_manager.get_session() as session:
            ...
            stmt = (
                ...
                .where(or_(Provider.last_health_check.is_(None), Provider.last_health_check < cutoff_time))
    ...
    async def bulk_update_health_status(self, provider_updates: list[dict[str, Any]]) -> int:
        ...
        async with self.db_manager.get_session():
            for update_data in provider_updates:
                ...
                if provider_id and health_status:
                    success = await self.update_health_status(provider_id, health_status, datetime.utcnow())
                    if success:
                        updated_count += 1
    ...
    async def search_providers(
        self, search_term: str, provider_type: str | None = None, enabled_only: bool = True
    ) -> list[Provider]:
    ...
    async def get_provider_statistics(self) -> dict[str, Any]:
        ...
        async with self.db_manager.get_session() as session:
            # Total providers
            total_stmt = select(func.count(Provider.id)).where(not Provider.is_deleted)
            total_result = await session.execute(total_stmt)
            total_providers = total_result.scalar() or 0

            # Enabled providers
            enabled_stmt = select(func.count(Provider.id)).where(and_(not Provider.is_deleted, Provider.is_enabled))
            ...
            healthy_stmt = select(func.count(Provider.id)).where(
                and_(not Provider.is_deleted, Provider.is_enabled, Provider.health_status == "healthy")
            )
