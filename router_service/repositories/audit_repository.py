...
class AuditRepository(BaseRepository[AuditLog]):
    ...
    async def get_by_event_type(
        self, event_type: str, tenant_id: str | None = None, page: int = 1, page_size: int = 100
    ) -> list[AuditLog]:
    ...
    async def get_by_user(
        self, user_id: str, tenant_id: str | None = None, page: int = 1, page_size: int = 100
    ) -> list[AuditLog]:
    ...
    async def get_recent_events(
        ...
        async with self.db_manager.get_session() as session:
            ...
            stmt = select(AuditLog).where(AuditLog.created_at >= cutoff_time)
    ...
    async def get_failed_events(
        self, tenant_id: str | None = None, page: int = 1, page_size: int = 100
    ) -> list[AuditLog]:
    ...
    async def get_security_events(
        ...
        async with self.db_manager.get_session() as session:
            stmt = select(AuditLog).where(AuditLog.event_type.in_(security_event_types))
    ...
    async def get_event_statistics(
        ...
        async with self.db_manager.get_session() as session:
            ...
            # Events by outcome
            outcome_stmt = select(AuditLog.outcome, func.count(AuditLog.id)).group_by(AuditLog.outcome)
            ...
            return {
                ...
                "success_rate": (events_by_outcome.get("success", 0) / total_events * 100 if total_events > 0 else 0),
            }

    async def get_user_activity_summary(
        self, user_id: str, tenant_id: str | None = None, hours: int = 24
    ) -> dict[str, Any]:
