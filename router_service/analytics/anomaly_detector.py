...
class AnomalyDetector:
    ...
    async def detect_anomalies(
        ...
        try:
            ...
            detection_results["summary"] = {
                ...
                "anomaly_types_detected": list(set(a.get("anomaly_type") for a in all_anomalies)),
                "most_common_anomaly_type": max(set(a.get("anomaly_type") for a in all_anomalies), 
                                               key=lambda x: sum(1 for a in all_anomalies if a.get("anomaly_type") == x)) if all_anomalies else None
            }
    ...
    async def _detect_statistical_anomalies(self, metrics_data: List[Dict[str, Any]], detection_types: List[str]) -> Dict[str, Any]:
        ...
        for anomaly_type in detection_types:
            ...
            for point in anomalous_points:
                ...
                anomaly = {
                    ...
                    "description": self._generate_anomaly_description(anomaly_type, point["value"], mean_value, threshold)
                }
        ...
        return {
            "method": "statistical",
            "anomalies": anomalies,
            "detection_types_checked": detection_types
        }
        ...
        for model, usage_data in model_usage_windows.items():
            ...
            for usage_point in usage_data[-3:]:  # Check last 3 data points
                ...
                if std_usage > 0 and deviation > std_usage * 3:
                    anomaly_type = "model_usage_spike" if usage_point["usage_count"] > mean_usage else "model_usage_drop"
    ...
    def _create_time_windows(self, metrics: List[Dict[str, Any]], window_minutes: int = 5) -> List[List[Dict[str, Any]]]:
    ...
    def _analyze_feature_contributions(self, anomalous_features: List[float], all_features: List[List[float]]) -> Dict[str, float]:
        """Analyze which features contributed most to the anomaly."""
        feature_names = [
            "response_time_ms", "cost_usd", "tokens_input", "tokens_output", 
            "quality_score", "error_flag", "model_hash", "user_hash"
        ]
        
        contributions = {}
        
        # Calculate how much each feature deviates from the norm
        for i, (feature_value, feature_name) in enumerate(zip(anomalous_features, feature_names)):
    ...
    def get_anomaly_history(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        recent_anomalies = [
            ...
            if anomaly.get("timestamp", 0) >= cutoff_time
        ]
