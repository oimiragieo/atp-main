...
class AnalyticsManager:
    ...
    def __init__(self, config: AnalyticsConfig | None = None):
        ...
        self.performance_analyzer = (
            PerformanceAnalyzer(self.config) if self.config.performance_analytics_enabled else None
        )
        self.business_intelligence = (
            BusinessIntelligenceEngine(self.config) if self.config.business_intelligence_enabled else None
        )
    ...
    async def run_comprehensive_analysis(
        ...
        try:
            ...
            if self.request_analyzer:
                analysis_tasks.append(self._run_request_analysis(request_data, time_window_hours))

            # Performance analysis
            if self.performance_analyzer and (performance_data or request_data):
                metrics_data = performance_data or request_data
                analysis_tasks.append(self._run_performance_analysis(metrics_data, time_window_hours))
            ...
            # Anomaly detection
            if self.anomaly_detector:
                analysis_tasks.append(self._run_anomaly_detection(request_data))

            # Trend analysis
            if self.trend_analyzer:
                analysis_tasks.append(self._run_trend_analysis(request_data))
    ...
    async def _run_performance_analysis(
        self, metrics_data: list[dict[str, Any]], time_window_hours: int
    ) -> dict[str, Any]:
    ...
    async def export_analytics_data(self, format_type: str = "json", time_range_hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            if self.insights_generator:
                export_data["data"]["insights_history"] = self.insights_generator.get_insights_history(
                    time_range_hours // 24
                )
...
def initialize_analytics_manager(config: AnalyticsConfig | None = None) -> AnalyticsManager:
