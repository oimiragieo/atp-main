...
class BusinessIntelligenceEngine:
    ...
    async def generate_business_insights(
        ...
        try:
            ...
            # Business KPIs
            insights["business_kpis"] = await self._calculate_business_kpis(
                recent_requests, recent_costs, recent_performance
            )
    ...
    async def _analyze_cost_trends(
        self, cost_data: list[dict[str, Any]], request_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        if len(all_costs) > 1:
            ...
            if cost_efficiency_trend:
                analysis["cost_efficiency"] = {
                    ...
                    "efficiency_trend": self._calculate_trend_direction(
                        [t["cost_per_request"] for t in cost_efficiency_trend]
                    ),
                }
        ...
        for model, daily_model_usage in model_usage.items():
            if len(daily_model_usage) >= 3:
                ...
                model_forecasts[model] = {
                    ...
                    "trend": model_trend,
                    "projected_30d_total": int(
                        model_avg
                        * 30
                        * (1.1 if model_trend == "increasing" else 0.9 if model_trend == "decreasing" else 1.0)
                    ),
                }
    ...
    async def _analyze_capacity_needs(
        self, request_data: list[dict[str, Any]], performance_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        current_analysis = {
            "current_load": {
                ...
                "time_span_hours": (
                    max(r.get("timestamp", 0) for r in request_data) - min(r.get("timestamp", 0) for r in request_data)
                )
                / 3600
                if request_data
                else 0,
        ...
        if len(request_data) > 24:  # At least 24 data points
            recent_load = len(
                [r for r in request_data if r.get("timestamp", 0) > time.time() - 7 * 24 * 3600]
            )  # Last 7 days
    ...
    async def _calculate_roi_metrics(
        self, request_data: list[dict[str, Any]], cost_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        unique_users = len({r.get("user_id") for r in request_data if r.get("user_id")})
        ...
        if total_cost > 0:
            roi_metrics["roi_calculation"] = {
                ...
                "break_even_requests": int(total_cost / estimated_value_per_request)
                if estimated_value_per_request > 0
                else 0,
    ...
    async def _calculate_business_kpis(
        ...
        if request_data:
            kpis["usage_kpis"] = {
                "total_requests": len(request_data),
                "unique_users": len({r.get("user_id") for r in request_data if r.get("user_id")}),
                "unique_tenants": len({r.get("tenant_id") for r in request_data if r.get("tenant_id")}),
                "requests_per_user": len(request_data)
                / len({r.get("user_id") for r in request_data if r.get("user_id")})
                if any(r.get("user_id") for r in request_data)
                else 0,
                "active_models": len({r.get("model_used") for r in request_data if r.get("model_used")}),
                "active_providers": len({r.get("provider_used") for r in request_data if r.get("provider_used")}),
            }
        ...
        if performance_data:
            ...
            if response_times:
                kpis["performance_kpis"] = {
                    ...
                    "sla_compliance_percentage": (sum(1 for rt in response_times if rt < 5000) / len(response_times))
                    * 100,
                }
        ...
        if total_cost > 0:
            kpis["cost_kpis"] = {
                "total_cost_usd": total_cost,
                "cost_per_request": total_cost / len(request_data) if request_data else 0,
                "cost_per_user": total_cost / len({r.get("user_id") for r in request_data if r.get("user_id")})
                if any(r.get("user_id") for r in request_data)
                else 0,
    ...
    def _create_time_windows_from_costs(
        self, cost_data: list[dict[str, Any]], window_hours: int = 24
    ...
    async def _analyze_market_trends(self, request_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if model_trends:
            ...
            market_analysis["model_trends"] = {
                ...
                "market_leaders": dict(
                    sorted(model_trends.items(), key=lambda x: x[1]["market_share"], reverse=True)[:5]
                ),
            }
        ...
        if provider_usage:
            ...
            for provider, usage in provider_usage.items():
                ...
                avg_performance = (
                    statistics.mean(provider_performance[provider]) if provider_performance[provider] else 0
                )
            ...
            for rank, (provider, _data) in enumerate(sorted_by_performance, 1):
        ...
        if hourly_usage:
            ...
            market_analysis["usage_patterns"] = {
                ...
                "usage_concentration": max(hourly_usage.values()) / statistics.mean(hourly_usage.values())
                if hourly_usage
                else 0,
            }
    ...
    async def _analyze_user_behavior_patterns(self, request_data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        user_segments = {"power_users": [], "regular_users": [], "occasional_users": [], "new_users": []}
        ...
        for user_id, metrics in user_metrics.items():
            ...
            user_profile = {
                ...
                "usage_span_days": (metrics["last_request"] - metrics["first_request"]) / (24 * 3600)
                if metrics["first_request"] and metrics["last_request"]
                else 0,
            }
        ...
        behavior_analysis["top_users"] = {
            ...
            "by_quality": sorted(
                [u for u in all_users if u["avg_quality_score"] > 0], key=lambda x: x["avg_quality_score"], reverse=True
            )[:10],
        }
        ...
        if user_segments["power_users"]:
            ...
            behavior_analysis["power_user_insights"] = {
                ...
                "cost_contribution": sum(u["total_cost"] for u in user_segments["power_users"])
                / sum(user_metrics[uid]["total_cost"] for uid in user_metrics)
                * 100,
                "request_contribution": sum(u["total_requests"] for u in user_segments["power_users"])
                / len(request_data)
                * 100,
            }
    ...
    def get_cached_insights(self) -> dict[str, Any] | None:
    ...
    def update_kpi_history(self, kpi_name: str, value: float, timestamp: float | None = None) -> None:
        """Update KPI history for trend tracking."""
        if timestamp is None:
            timestamp = time.time()

        self._kpi_history[kpi_name].append({"timestamp": timestamp, "value": value})
    ...
    def get_kpi_trend(self, kpi_name: str, days: int = 30) -> dict[str, Any] | None:
        """Get KPI trend for specified time period."""
        if kpi_name not in self._kpi_history:
            return None

        cutoff_time = time.time() - (days * 24 * 3600)
        recent_data = [entry for entry in self._kpi_history[kpi_name] if entry["timestamp"] >= cutoff_time]
