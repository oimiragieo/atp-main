...
class ATPTestRunner:
    ...
    async def run_test_suite(self, suite_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *suite.command, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=os.getcwd()
            )
    ...
    async def run_all_tests(self, parallel: bool = False, exclude: list[str] | None = None) -> dict[str, Any]:
        """Run all test suites."""
        exclude = exclude or []
...
"""
        ...
        for _suite_name, result in test_results.items():
...
def main():
    ...
    try:
        if args.suites:
            ...
            asyncio.set_event_loop(loop)
            try:
                summary = loop.run_until_complete(runner.run_all_tests(parallel=args.parallel, exclude=args.exclude))
            finally:
                loop.close()
        ...
        # Print failed suites
        failed_suites = [name for name, result in summary["test_results"].items() if not result.get("success", False)]
