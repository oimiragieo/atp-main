...
class CoverageAnalyzer:
        ...
        for filename in cov.get_data().measured_files():
            if self._should_include_file(filename):
                ...
                for func_name, line_start, line_end in functions:
                    ...
                    file_functions.append(FunctionCoverage(
                        name=func_name,
            ...
            if self._should_include_file(filename):
                ...
                for class_name, line_start, line_end, methods in classes:
                    ...
                    for method_name, method_start, method_end in methods:
                        ...
                        method_coverage_pct = (method_lines_covered / method_lines_total * 100) if method_lines_total > 0 else 100
                        
                        method_coverages.append(FunctionCoverage(
                            name=method_name,
                    ...
                    file_classes.append(ClassCoverage(
                        name=class_name,
                        line_start=line_start,
                        line_end=line_end,
    ...
    def _generate_recommendations(self, cov: coverage.Coverage) -> List[Dict[str, Any]]:
        ...
        file_metrics = self._analyze_file_coverage(cov)
        low_coverage_files = [
            (filename, metrics) for filename, metrics in file_metrics.items()
...
"""
        ...
        for filename, metrics in analysis_results['file_metrics'].items():
            coverage_class = "high" if metrics.coverage_percentage >= 80 else "medium" if metrics.coverage_percentage >= 60 else "low"
            html_content += f"""
        <tr>
