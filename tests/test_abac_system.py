...
import pytest
from router_service.enterprise_auth import AuthProvider, UserInfo
...
class TestPolicyRule:
    ...
    def test_rule_evaluation(self):
        rule = PolicyRule(
            ...
            conditions=[AttributeCondition("user.roles", Operator.CONTAINS, "admin")],
            resources=["api/*"],
            actions=["read", "write"],
        )
...
class TestABACPolicy:
        ...
        policy = ABACPolicy(
            ...
            rules=[
                ...
                PolicyRule(
                    ...
                    effect=Effect.PERMIT,
                    conditions=[AttributeCondition("user.roles", Operator.CONTAINS, "user")],
                    actions=["read"],
                ),
            ],
        )
    ...
    def test_disabled_policy(self):
        policy = ABACPolicy(
            ...
            enabled=False,
            rules=[
                PolicyRule(rule_id="permit_all", description="Permit everything", effect=Effect.PERMIT, conditions=[])
            ],
        )
...
class TestEnhancedPolicyEngine:
    ...
    def test_add_and_get_policy(self):
        policy = ABACPolicy(policy_id="test_policy", name="Test Policy", description="Test policy", rules=[])
    ...
    def test_policy_priority_evaluation(self):
        ...
        deny_policy = ABACPolicy(
            ...
            rules=[
                PolicyRule(
                    ...
                    effect=Effect.DENY,
                    conditions=[AttributeCondition("user.roles", Operator.CONTAINS, "blocked")],
                )
            ],
        )
        ...
        ctx = Context(user_id="test_user", roles={"blocked"}, resource="api/data", action="read")
    ...
    def test_policy_caching(self):
        ...
        ctx = Context(user_id="admin_user", roles={"admin"}, resource="api/data", action="read")
...
class TestTenantIsolation:
    ...
    def test_tenant_scoped_service(self):
        ...
        # Mock user info
        user_info = UserInfo(
            user_id="user123", email="user@example.com", name="Test User", roles={"user"}, tenant_id="tenant1"
        )
        ...
        # Test admin user
        admin_user = UserInfo(
            user_id="admin123", email="admin@example.com", name="Admin User", roles={"admin"}, tenant_id="tenant1"
        )
...
class TestPolicyAPI:
    ...
    async def test_create_policy_endpoint(self):
        from router_service.enterprise_auth import UserInfo

        from router_service.policy_api import ABACPolicyModel, AttributeConditionModel, PolicyRuleModel

        # Mock user
        UserInfo(
            user_id="admin", email="admin@test.com", name="Admin", roles={"admin"}, provider=AuthProvider.ADMIN_KEYS
        )
        ...
        policy_data = ABACPolicyModel(
            ...
            rules=[
                PolicyRuleModel(
                    ...
                    effect=Effect.PERMIT,
                    conditions=[
                        AttributeConditionModel(attribute="user.roles", operator=Operator.CONTAINS, value="admin")
                    ],
                )
            ],
        )
...
def test_legacy_compatibility():
    ...
    from router_service.policy_engine import Context, Policy
