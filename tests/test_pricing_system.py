...
async def test_mock_pricing_api():
    ...
    # Set test data
    test_pricing = {"test-model": {"input": 0.01, "output": 0.03}}
...
async def test_pricing_validation():
    ...
    validation_result = await monitor.validate_pricing_accuracy(
        provider="openai", model="gpt-4", actual_cost=0.08, tokens_used=1000, token_type="input"
    )
...
async def test_pricing_manager_cost_optimization():
    ...
    # Mock usage data
    usage_data = {"openai": {"gpt-4": 10000, "gpt-3.5-turbo": 5000}, "anthropic": {"claude-3-opus": 2000}}
