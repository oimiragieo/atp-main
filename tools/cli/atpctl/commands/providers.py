...
def describe_provider(
    ctx: typer.Context,
    provider_name: str = typer.Argument(..., help="Name of the provider to describe")
):
...
def add_provider(
    ...
    endpoint: Optional[str] = typer.Option(None, "--endpoint", help="Provider endpoint URL"),
...
def update_provider(
    ...
    endpoint: Optional[str] = typer.Option(None, "--endpoint", help="Provider endpoint URL"),
    api_key: Optional[str] = typer.Option(None, "--api-key", help="API key"),
    priority: Optional[int] = typer.Option(None, "--priority", help="Provider priority"),
    enabled: Optional[bool] = typer.Option(None, "--enabled/--disabled", help="Enable/disable provider")
):
    ...
    try:
        ...
        response = client.put(f"/api/v1/providers/{provider_name}", json=current_provider)
...
def test_provider(
    ctx: typer.Context,
    provider_name: str = typer.Argument(..., help="Provider name"),
    model: Optional[str] = typer.Option(None, "--model", help="Specific model to test"),
    ...
    try:
        ...
        if response.get('success'):
            rprint(f"[green]✓ Provider test successful[/green]")
            ...
            rprint(f"[red]✗ Provider test failed[/red]")
...
def check_health(
    ctx: typer.Context,
    provider_name: Optional[str] = typer.Argument(None, help="Provider name (check all if not specified)")
):
...
def list_models(
    ctx: typer.Context,
    provider_name: Optional[str] = typer.Argument(None, help="Provider name (list all if not specified)"),
