...
from ..utils.validators import validate_node_config
...
def list_nodes(
    ...
    labels: Optional[str] = typer.Option(None, "--labels", "-l", help="Filter by labels (key=value)")
):
    """List cluster nodes"""
    try:
...
def describe_node(
    ctx: typer.Context,
    node_name: str = typer.Argument(..., help="Name of the node to describe")
):
...
def scale_cluster(
    ...
    component: Optional[str] = typer.Option("router", "--component", "-c", help="Component to scale"),
    namespace: Optional[str] = typer.Option("default", "--namespace", "-n", help="Namespace")
):
...
def cordon_node(
    ctx: typer.Context,
    node_name: str = typer.Argument(..., help="Name of the node to cordon")
):
    """Cordon a cluster node (mark as unschedulable)"""
    try:
        client = ATPAPIClient.from_context(ctx)
        response = client.post(f"/api/v1/cluster/nodes/{node_name}/cordon")
...
def uncordon_node(
    ctx: typer.Context,
    node_name: str = typer.Argument(..., help="Name of the node to uncordon")
):
    """Uncordon a cluster node (mark as schedulable)"""
    try:
        client = ATPAPIClient.from_context(ctx)
        response = client.post(f"/api/v1/cluster/nodes/{node_name}/uncordon")
...
def get_logs(
    ...
    tail: Optional[int] = typer.Option(100, "--tail", help="Number of lines to show"),
...
def restore_cluster(
    ...
    try:
        ...
        rprint(f"[yellow]Restoring cluster from backup...[/yellow]")
