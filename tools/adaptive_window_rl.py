...
class AdaptiveWindowRLAgent:
    ...
    def get_state(self, session: str, latency_ms: float, throughput: float, error_rate: float) -> RLState:
    ...
    def select_action(self, state: RLState) -> int:
        ...
        if random.random() < self.exploration_rate:
            ...
            action_idx = max(range(len(self.action_space)), key=lambda i: self.q_table[state_tuple][i])
    ...
    def calculate_reward(self, state: RLState, prev_state: RLState | None = None) -> float:
    ...
    def update_q_table(self, state: RLState, action_idx: int, reward: float, next_state: RLState) -> None:
        """Update Q-table using Q-learning update rule."""
        state_tuple = self.discretize_state(state)
        next_state_tuple = self.discretize_state(next_state)

        # Find best action for next state
        best_next_action = max(range(len(self.action_space)), key=lambda i: self.q_table[next_state_tuple][i])
    ...
    def learn_from_feedback(
        self, session: str, latency_ms: float, throughput: float, error_rate: float, success: bool = True
    ) -> None:
