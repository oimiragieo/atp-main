...
class DependencyAnalyzer:
    ...
    def _find_unused_imports(self):
        ...
        for _module_name, analysis in self.module_analyses.items():
            try:
        ...
        for module_name, analysis in self.module_analyses.items():
            ...
            try:
                if module_name in self.dependency_graph:
                    ...
                    leaf_nodes = [n for n in self.dependency_graph.nodes() if self.dependency_graph.out_degree(n) == 0]
    ...
    def _generate_report(self) -> dict[str, any]:
        ...
        internal_imports = sum(
            len([imp for imp in a.imports if not imp.is_external]) for a in self.module_analyses.values()
        )
        external_imports = total_imports - internal_imports

        # Find modules with issues
        modules_with_cycles = [
            name for name, analysis in self.module_analyses.items() if analysis.circular_dependencies
        ]

        modules_with_unused = [name for name, analysis in self.module_analyses.items() if analysis.unused_imports]
        ...
        if self.dependency_graph.nodes():
            ...
            highly_depended_on = sorted(in_degrees.items(), key=lambda x: x[1], reverse=True)[:10]

            highly_dependent = sorted(out_degrees.items(), key=lambda x: x[1], reverse=True)[:10]
        else:
            highly_depended_on = []
            highly_dependent = []
        ...
        top_external_deps = sorted(external_dep_counts.items(), key=lambda x: x[1], reverse=True)[:15]
        ...
        report = {
            ...
            "highly_connected": {"most_depended_on": highly_depended_on, "most_dependent": highly_dependent},
            "external_dependencies": {
                "top_dependencies": top_external_deps,
                "all_dependencies": sorted(self.external_dependencies),
            },
            "detailed_analysis": {
                module_name: asdict(analysis) for module_name, analysis in self.module_analyses.items()
            },
        }
    ...
    def generate_cleanup_recommendations(self) -> list[str]:
        ...
        if len(external_deps) > 50:
            recommendations.append(f"Consider consolidating {len(external_deps)} external dependencies")
...
def main():
    ...
    analyzer.save_report("tools/cleanup/dependency_analysis.json")
