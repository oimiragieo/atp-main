...
class SecurityScanner:
    ...
    def __init__(self, root_path: str = "."):
        ...
        self.suspicious_patterns = {
            ...
            "eval_usage": {"pattern": r"\beval\s*\(", "severity": "high", "description": "Use of eval() function"},
            "exec_usage": {"pattern": r"\bexec\s*\(", "severity": "high", "description": "Use of exec() function"},
        }
    ...
    def _scan_file(self, file_path: Path) -> FileSecurityAnalysis:
        ...
        try:
            ...
            all_patterns = {**self.secret_patterns, **self.pii_patterns, **self.suspicious_patterns}
    ...
    def _generate_report(self) -> dict[str, any]:
        ...
        high_risk_files = [analysis for analysis in self.file_analyses if analysis.risk_score >= 7]

        # Find files with secrets
        files_with_secrets = [analysis for analysis in self.file_analyses if analysis.contains_secrets]

        # Find files with PII
        files_with_pii = [analysis for analysis in self.file_analyses if analysis.contains_pii]

        # Find test files with sensitive data
        test_files_with_sensitive_data = [analysis for analysis in self.file_analyses if analysis.contains_test_data]

        # Top issue types
        top_issue_types = sorted(issue_type_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    def generate_remediation_plan(self) -> list[str]:
        ...
        for issue_type, count in report["top_issue_types"][:5]:
            if count > 5:  # Only recommend for frequent issues
                remediation_steps.append(f"Address {count} instances of {issue_type.replace('_', ' ')}")
...
def main():
    ...
    if critical_files:
        ...
        print("Critical files list saved to: tools/cleanup/critical_security_files.txt")
