...
class FileClassifier:
    ...
    def __init__(self, root_path: str = "."):
        ...
        self.debug_patterns = [
            "debug_*.py", "temp_*.py", "*.tmp", "*_debug.py",
            "test_debug_*.py", "debug*.log", "*.debug"
        ]
        
        self.temp_patterns = [
            "temp_*.py", "*.tmp", "*.temp", "*_temp.*",
            "temporary_*", "scratch_*", "*.bak", "*.backup"
        ]
        ...
        self.config_patterns = [
            ...
            "*.env", "*.conf", "*.config", "Dockerfile*",
            "docker-compose*.yml", "*.tf", "*.tfvars"
        ]
        ...
        self.core_service_patterns = [
            "router_service/*.py", "memory-gateway/*.py",
            "adapters/*/adapter.py", "services/*/*.py"
        ]
    ...
    def _calculate_security_risk(self, full_path: Path, file_path: str) -> int:
        ...
        try:
            if full_path.is_file() and full_path.suffix in ['.py', '.js', '.ts', '.yml', '.yaml', '.json', '.env']:
                ...
                if 'test' in file_path.lower() and any(keyword in content.lower() for keyword in ['email', 'phone', 'address']):
    ...
    def _determine_cleanup_action(self, file_path: str, category: str, 
                                production_relevance: int, security_risk: int) -> Tuple[str, Optional[str], str]:
        """Determine what cleanup action to take for a file."""
        
        # Remove debug and temp files
        if category in ["debug", "temp"]:
            return "remove", None, f"Debug/temporary file not needed for production"
        ...
        if security_risk >= 5 and production_relevance >= 7:
            return "refactor", None, f"Contains security risks but needed for production"
    ...
    def _generate_report(self) -> Dict[str, any]:
        ...
        high_risk_files = [
            ...
            if c.security_risk >= 7
        ]
        
        # Find files to remove
        files_to_remove = [
            c for c in self.classifications 
            if c.cleanup_action == "remove"
        ]
        
        # Find files to relocate
        files_to_relocate = [
            c for c in self.classifications 
            if c.cleanup_action == "relocate"
        ]
    ...
    def print_summary(self):
        ...
        print(f"  Size reduction: {report['estimated_size_reduction']['mb']} MB ({report['estimated_size_reduction']['percentage']}%)")
...
def main():
    ...
    files_to_remove = [
        c['file_path'] for c in report['detailed_classifications']
        if c['cleanup_action'] == 'remove'
    ]
    ...
    files_to_relocate = [
        (c['file_path'], c['target_path']) for c in report['detailed_classifications']
    ...
    print(f"Files to relocate list saved to: tools/cleanup/files_to_relocate.txt")
