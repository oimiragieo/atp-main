...
class SLATierSpecification:
    ...
    def _initialize_default_tiers(self):
        ...
        platinum_slo = SLODefinition(
            sli_targets=platinum_sli, compliance_window_days=30, alert_threshold_pct=0.95, burn_rate_threshold=2.0
        )
        ...
        gold_slo = SLODefinition(
            sli_targets=gold_sli, compliance_window_days=30, alert_threshold_pct=0.90, burn_rate_threshold=1.5
        )
        self.tiers[ServiceTier.GOLD] = SLATier(
            ...
            custom_features=["High availability", "Business hours support", "Enhanced monitoring", "Priority queuing"],
        )
        ...
        silver_slo = SLODefinition(
            sli_targets=silver_sli, compliance_window_days=30, alert_threshold_pct=0.85, burn_rate_threshold=1.2
        )
        self.tiers[ServiceTier.SILVER] = SLATier(
            ...
            custom_features=["Standard availability", "Email support", "Basic monitoring"],
        )
        ...
        bronze_slo = SLODefinition(
            sli_targets=bronze_sli, compliance_window_days=30, alert_threshold_pct=0.80, burn_rate_threshold=1.0
        )
        self.tiers[ServiceTier.BRONZE] = SLATier(
            ...
            custom_features=["Basic availability", "Community support", "Limited monitoring"],
        )
        ...
        basic_slo = SLODefinition(
            sli_targets=basic_sli, compliance_window_days=30, alert_threshold_pct=0.75, burn_rate_threshold=0.8
        )
        self.tiers[ServiceTier.BASIC] = SLATier(
            ...
            custom_features=["Best effort availability", "Community support only", "Minimal monitoring"],
        )
...
Examples:
  ...
  python sla_tier_specification.py --show-tiers
    ...
    parser.add_argument("--export-catalog", help="Export complete SLA catalog to JSON file")
    ...
    parser.add_argument("--metrics-file", help="JSON file containing current metrics data for compliance evaluation")

    parser.add_argument("--show-tiers", action="store_true", help="Display all tier specifications")

    parser.add_argument("--output", help="Output file for generated configurations")
