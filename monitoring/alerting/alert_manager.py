...
Runbook: {alert.annotations.get('runbook', 'N/A')}
                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            {"title": "Fired At", "value": datetime.fromtimestamp(alert.fired_at).isoformat(), "short": True}
                        ]
                    }
                ]
            }
    ...
    def update_incident_status(self, incident_id: str, status: IncidentStatus, user: str, notes: Optional[str] = None):
    ...
    def _update_mtta(self, time_to_acknowledge: float):
        ...
        if total_alerts > 1:
            self.alert_metrics["mean_time_to_acknowledge"] = (
                (current_mtta * (total_alerts - 1) + time_to_acknowledge) / total_alerts
            )
        else:
    ...
    def _update_mttr(self, time_to_resolve: float):
        ...
        if resolved_count > 1:
            self.alert_metrics["mean_time_to_resolve"] = (
                (current_mttr * (resolved_count - 1) + time_to_resolve) / resolved_count
            )
        else:
