...
class VertexAITrainingPipeline:
    ...
    async def _get_job_status(self, job_id: str) -> TrainingJob | None:
    ...
    async def submit_training_job(self, config: TrainingConfig) -> str:
        ...
        try:
            ...
            if config.job_type == TrainingJobType.CUSTOM_TRAINING:
                await self._submit_custom_training_job(config)
            elif config.job_type == TrainingJobType.AUTOML:
                await self._submit_automl_job(config)
            elif config.job_type == TrainingJobType.HYPERPARAMETER_TUNING:
                await self._submit_hp_tuning_job(config)
            else:
    ...
    async def _submit_hp_tuning_job(self, config: TrainingConfig) -> Any:
        ...
        job = aiplatform.HyperparameterTuningJob(
            ...
            metric_spec={
                config.training_task_inputs["optimization_goal_metric"]: config.training_task_inputs[
                    "optimization_goal"
                ]
            },
    ...
    async def list_jobs(
        self,
        status_filter: TrainingJobStatus | None = None,
        job_type_filter: TrainingJobType | None = None,
        limit: int = 50,
    ) -> list[dict[str, Any]]:
