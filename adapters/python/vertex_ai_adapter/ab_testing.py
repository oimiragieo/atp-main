...
class ExperimentVariant:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "metrics": {k: {"count": len(v), "mean": np.mean(v), "std": np.std(v)} for k, v in self.metrics.items()},
        }
...
class VertexAIABTesting:
    ...
    async def _collect_experiment_metrics(self, experiment: Experiment):
        ...
        for variant_id, variant in experiment.variants.items():
            try:
                ...
                metrics = await self.monitoring_system.get_model_metrics(variant.model_id, time_range_hours=1)
    ...
    async def _check_early_stopping(self, experiment: Experiment) -> bool:
        ...
        try:
            ...
            for _treatment_id, significance in result.statistical_significance.items():
    ...
    async def create_experiment(self, config: ExperimentConfig) -> str:
        ...
        try:
            ...
            experiment = Experiment(
                config=config, status=ExperimentStatus.DRAFT, variants=variants, created_at=time.time()
            )
    ...
    async def start_experiment(self, experiment_id: str, endpoint_name: str) -> bool:
        ...
        try:
            ...
            traffic_split = {variant.model_id: variant.traffic_percentage for variant in experiment.variants.values()}
    ...
    async def _analyze_experiment(self, experiment: Experiment) -> ExperimentResult:
        ...
        for treatment_variant in treatment_variants:
            ...
            test_result = self._perform_statistical_test(control_values, treatment_values, StatisticalTest.T_TEST)

            statistical_significance[treatment_variant.variant_id] = test_result

            # Calculate confidence interval
            ci = self._calculate_confidence_interval(treatment_values, experiment.config.significance_level)
    ...
    def _perform_statistical_test(
        self, control_values: list[float], treatment_values: list[float], test_type: StatisticalTest
    ) -> dict[str, Any]:
        """Perform statistical test."""
        ...
        try:
            if test_type == StatisticalTest.T_TEST:
                ...
                return {"test_type": "mann_whitney", "statistic": float(statistic), "p_value": float(p_value)}
            ...
            return {"test_type": test_type.value, "error": str(e), "p_value": 1.0}

    def _calculate_confidence_interval(self, values: list[float], significance_level: float) -> tuple[float, float]:
        """Calculate confidence interval."""
    ...
    async def list_experiments(self, status_filter: ExperimentStatus | None = None) -> list[dict[str, Any]]:
