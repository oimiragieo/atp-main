...
from google.cloud.monitoring_v3.types import (
    TimeSeries, Metric, MetricDescriptor, AlertPolicy, NotificationChannel
)
...
class VertexAIMonitoring:
    ...
    async def _collect_model_metrics(self, config: MonitoringConfig) -> Optional[ModelPerformanceMetrics]:
    ...
    async def get_active_alerts(self, model_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get active alerts."""
        alerts = []
    ...
    async def get_model_health_score(self, model_id: str) -> Dict[str, Any]:
        ...
        recent_metrics = [
            metrics for metrics in self.performance_history[model_id]
            if metrics.timestamp >= time.time() - 3600
        ]
