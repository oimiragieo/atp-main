...
class VertexAIModelManager:
    ...
    async def deploy_model(self, config: ModelDeploymentConfig, endpoint_name: str | None = None) -> tuple[str, str]:
        ...
        try:
            ...
            if endpoint_name and endpoint_name in self.endpoints:
                ...
                endpoint = aiplatform.Endpoint.create(
                    display_name=endpoint_display_name, enable_request_response_logging=config.enable_access_logging
                )
    ...
    async def update_traffic_split(self, endpoint_name: str, traffic_split: TrafficSplit) -> bool:
    ...
    def _get_deployed_model_id(self, endpoint_name: str, version_id: str) -> str | None:
    ...
    async def scale_model(self, endpoint_name: str, version_id: str, min_replicas: int, max_replicas: int) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Scaled model {version_id} on endpoint {endpoint_name} to {min_replicas}-{max_replicas} replicas"
            )
            return True
    ...
    async def get_model_metrics(
        self, endpoint_name: str, version_id: str | None = None, time_range_hours: int = 24
    ) -> dict[str, Any]:
        ...
        try:
            ...
            request_count = await self._query_metric(
                project_name, "aiplatform.googleapis.com/prediction/online/request_count", endpoint.name, interval
            )
            metrics["request_count"] = request_count

            # Latency
            latency = await self._query_metric(
                project_name, "aiplatform.googleapis.com/prediction/online/response_latencies", endpoint.name, interval
            )
            ...
            error_count = await self._query_metric(
                project_name, "aiplatform.googleapis.com/prediction/online/error_count", endpoint.name, interval
            )
            ...
            cpu_utilization = await self._query_metric(
                project_name, "aiplatform.googleapis.com/prediction/online/cpu_utilization", endpoint.name, interval
            )
            metrics["cpu_utilization"] = cpu_utilization

            memory_utilization = await self._query_metric(
                project_name, "aiplatform.googleapis.com/prediction/online/memory_utilization", endpoint.name, interval
            )
    ...
    async def create_canary_deployment(
        ...
        try:
            ...
            model_name, endpoint_id = await self.deploy_model(config=new_config, endpoint_name=endpoint_name)
    ...
    async def promote_canary(self, endpoint_name: str, canary_version_id: str, target_percentage: int = 100) -> bool:
        ...
        try:
            ...
            if target_percentage < 100:
                ...
                for model_versions in self.model_versions.values():
                    for version in model_versions:
                        if (
                            version.endpoint_id.endswith(endpoint_name)
                            and version.version_id != canary_version_id
                            and version.status == ModelStatus.DEPLOYED
                        ):
    ...
    async def rollback_deployment(self, endpoint_name: str, target_version_id: str) -> bool:
