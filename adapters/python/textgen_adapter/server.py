...
class TextGenWebUIClient:
    ...
    def __init__(self, host: str = DEFAULT_TEXTGEN_HOST, port: int = DEFAULT_TEXTGEN_PORT, ws_port: int = DEFAULT_TEXTGEN_WS_PORT):
        self.host = host
        self.port = port
    ...
    async def load_model(self, model_name: str) -> bool:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/api/v1/internal/model/load",
                json=payload
            ) as response:
                return response.status == 200
...
class TextGenWebUIAdapter:
    ...
    def __init__(self):
        ...
        # Initialize client
        self.textgen_client = TextGenWebUIClient(
            self.textgen_host, 
            self.textgen_port, 
            self.textgen_ws_port
        )
    ...
    async def Health(self, req: adapter_pb2.HealthRequest, ctx: Any) -> adapter_pb2.HealthResponse:  # noqa: N802
        ...
        try:
            ...
            return adapter_pb2.HealthResponse(
                p95_ms=150.0 if healthy else 5000.0,
                error_rate=0.02 if healthy else 0.8
            )
...
app = FastAPI(
    title="ATP Text Generation WebUI Adapter",
    description="Text Generation WebUI adapter for ATP",
    version="1.0.0"
)
...
async def get_available_models():
    ...
    try:
        ...
        return {
            "models": models,
            "current_model": current_model.get("result", "unknown")
        }
    except Exception as e:
