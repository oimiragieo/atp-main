...
class GPUMonitor:
    ...
    def get_optimal_gpu(self) -> int | None:
...
class BatchProcessor:
    ...
    async def _process_model_batch(self, model: str, requests: list[BatchRequest]):
        ...
        try:
            ...
            # Send responses
            for request, response_text in zip(requests, responses, strict=False):
    ...
    async def generate(
        ...
        try:
            ...
            async with self.session.post(
                f"{self.base_url}/v1/completions", json=payload, timeout=aiohttp.ClientTimeout(total=300)
            ) as response:
                if response.status == 200:
                    data = await response.json()
        ...
        for prompt in prompts:
            try:
                result = await self.generate(
                    prompt=prompt, model=model, max_tokens=max_tokens, temperature=temperature, top_p=top_p, stop=stop
                )
...
class VLLMAdapter:
        ...
        try:
            ...
            if use_batch:
                ...
                if batch_response.error:
                    yield adapter_pb2.StreamChunk(text="", is_final=True, error=batch_response.error)
                else:
                    ...
                    for i in range(0, len(text), chunk_size):
                        ...
                        yield adapter_pb2.StreamChunk(
                            type="text", content_json=json.dumps({"text": chunk}), confidence=0.9, more=not is_final
                        )
    ...
    async def Health(self, req: adapter_pb2.HealthRequest, ctx: Any) -> adapter_pb2.HealthResponse:  # noqa: N802
        ...
        try:
            ...
            return adapter_pb2.HealthResponse(p95_ms=100.0 if healthy else 5000.0, error_rate=0.01 if healthy else 0.5)
...
app = FastAPI(title="ATP vLLM Adapter", description="High-performance vLLM adapter for ATP", version="1.0.0")
...
async def get_gpu_stats():
    ...
    return {"gpu_count": len(gpu_stats), "gpus": [asdict(gpu) for gpu in gpu_stats]}
