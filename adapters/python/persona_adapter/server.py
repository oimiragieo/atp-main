...
"""ATP Persona Adapter Service.

A gRPC-based adapter service that provides persona-based AI model interactions.
This adapter simulates persona-aware responses for the ATP system.
...
class Adapter:  # thin wrapper implementing expected methods
    """Persona adapter implementation providing AI model interactions."""
    ...
    async def Stream(self, req: adapter_pb2.StreamRequest, ctx: Any) -> AsyncIterator[adapter_pb2.StreamChunk]:  # noqa: N802
        ...
        for i in range(3):
            ...
            yield adapter_pb2.StreamChunk(
                ...
                content_json=json.dumps({"chunk": i, "adapter": "persona_adapter"}),
                confidence=0.6,
        ...
        yield adapter_pb2.StreamChunk(
            ...
            content_json=json.dumps({"ok": True, "adapter": "persona_adapter"}),
            confidence=0.8,
...
def _register(server: grpc.aio.Server) -> None:
    # delegate to generated helper; Adapter matches required interface
