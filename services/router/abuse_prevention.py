...
class RateLimiter:
    ...
    def is_allowed(self, tenant_id: str, user_id: str = None, 
                   endpoint: str = "default") -> Tuple[bool, str, int]:
...
class LoopDetector:
    ...
    def start_request(self, request_context: RequestContext) -> Tuple[bool, Optional[str]]:
        ...
        for active_req in self.active_requests.values():
            if active_req.signature == signature:
                ...
                return False, f"Immediate loop detected: duplicate active request"
        
        # Check for pattern loops in recent history
        tenant_history = self.request_history[signature.tenant_id]
        recent_requests = [
            req for req in tenant_history 
            if (datetime.now() - req.timestamp).total_seconds() < self.loop_window
        ]
        ...
        if request_id in self.active_requests:
            ...
            if METRICS_AVAILABLE:
                _GAUGE_ACTIVE_REQUESTS.labels(tenant_id=context.signature.tenant_id).set(
                    len([r for r in self.active_requests.values() if r.signature.tenant_id == context.signature.tenant_id])
                )
...
class AbusePreventionSystem:
    ...
    def check_request(self, 
                     ...
                     user_agent: Optional[str] = None,
                     parent_request_id: Optional[str] = None,
                     depth: int = 0) -> Tuple[bool, Optional[BlockReason], str]:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_abuse_events": len([
                e for e in self.abuse_events
                if (datetime.now() - e.timestamp).total_seconds() < 3600
            ]),
            "anomaly_scores": dict(self.anomaly_detector.anomaly_scores)
        }
    
    def reset_entity(self, tenant_id: str, user_id: Optional[str] = None):
...
def check_request_abuse(request_id: str,
                       tenant_id: str,
                       endpoint: str,
                       method: str,
                       content: str,
...
def get_abuse_statistics(tenant_id: Optional[str] = None) -> Dict[str, Any]:
