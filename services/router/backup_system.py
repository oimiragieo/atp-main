...
class DatabaseBackupManager:
    ...
    async def create_backup(
        ...
        try:
            ...
            # Update metrics
            self.backup_counter.inc(1, {
                "policy_id": policy.id,
                "backup_type": policy.backup_type.value,
                "status": "success"
            })
            
            self.backup_duration.observe(backup_record.duration_seconds, {
                "policy_id": policy.id,
                "backup_type": policy.backup_type.value
            })
            
            self.backup_size.observe(backup_record.size_bytes, {
                "policy_id": policy.id,
                "backup_type": policy.backup_type.value
            })
    ...
    async def _encrypt_backup(
        ...
        if encryption_type == EncryptionType.AES256:
            ...
            encrypted_data, encrypted_key = await self.encryption_manager.encrypt_with_kms(
                data, kms_key_id
            )
...
class BackupScheduler:
    ...
    def __init__(
        ...
        redis_backup_manager: RedisBackupManager
    ):
    ...
    def _scheduler_loop(self):
        ...
        while self.scheduler_active:
            try:
                ...
                for policy_id, policy in self.backup_policies.items():
    ...
    def _get_last_backup_time(self, policy_id: str) -> Optional[datetime]:
        """Get the last backup time for a policy."""
        
        last_time = None
        for record in self.backup_records.values():
            if (record.policy_id == policy_id and 
                record.status == BackupStatus.COMPLETED and
                (not last_time or record.started_at > last_time)):
    ...
    async def _execute_backup(self, policy: BackupPolicy):
        ...
        try:
            if "database" in policy.name.lower() or "db" in policy.name.lower():
                # Database backup
                backup_record = await self.db_backup_manager.create_backup(
                    policy, self.database_url
                )
            elif "redis" in policy.name.lower():
                # Redis backup
                backup_record = await self.redis_backup_manager.create_backup(
                    policy, self.redis_url
                )
            else:
    ...
    async def trigger_backup(self, policy_id: str) -> str:
        ...
        for record in self.backup_records.values():
            if (record.policy_id == policy_id and
                (not latest_record or record.started_at > latest_record.started_at)):
    ...
    def list_backups(self, policy_id: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
        """List backups."""
