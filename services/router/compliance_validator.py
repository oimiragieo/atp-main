...
class ComplianceValidator:
    ...
    async def run_compliance_check(self, framework: Optional[ComplianceFramework] = None) -> ComplianceReport:
    ...
    async def check_data_retention(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
    ...
    async def check_consent_tracking(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
    ...
    async def check_data_subject_rights(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            test_response = await self.http_client.get(
                f"{self.memory_gateway_url}/v1/compliance/gdpr/data-subject/test",
                headers={"x-tenant-id": "system"}
            )
            ...
            return {
                "description": "Data subject rights endpoints not available",
                "metadata": {"error": str(e)}
            }
    
    async def check_access_control(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
    ...
    async def check_audit_logging(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
        """Check SOC 2 audit logging compliance."""
        try:
            # Check if audit log integrity is maintained
            response = await self.http_client.get(
                f"{self.memory_gateway_url}/v1/compliance/audit-integrity",
                headers={"x-tenant-id": "system"}
            )
            ...
            return {
                "description": "Audit logging system not accessible",
                "metadata": {"error": str(e)}
            }
        
        return None
    
    async def check_data_encryption(self, rule: ComplianceRule) -> Optional[Dict[str, Any]]:
        """Check SOC 2 data encryption compliance."""
        # Check if TLS is enforced
        encryption_issues = []
        
        # Check if HTTPS is enforced (basic check)
        if not os.getenv("FORCE_HTTPS", "").lower() in ("true", "1"):
            encryption_issues.append("HTTPS not enforced")
        
        # Check if database encryption is configured
        if not os.getenv("DB_ENCRYPTION_ENABLED", "").lower() in ("true", "1"):
            ...
            try:
                ...
                for framework in ComplianceFramework:
                    try:
                        ...
                        logger.info(f"Background compliance check completed for {framework.value}: "
    ...
    def get_violations(self, framework: Optional[ComplianceFramework] = None) -> List[ComplianceViolation]:
