...
class TrendAnalyzer:
    ...
    async def analyze_trends(
        self, metrics_data: list[dict[str, Any]], metric_types: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if len(recent_metrics) < self._min_data_points:
                return {
                    "error": f"Insufficient data for trend analysis (minimum {self._min_data_points} points required)"
                }
    ...
    async def _analyze_metric_trend(
        self, metrics_data: list[dict[str, Any]], metric_type: str
    ) -> dict[str, Any] | None:
        ...
        if metric_type == "response_time":
            ...
            for metric in metrics_data:
                if metric.get("response_time_ms") is not None:
                    time_series.append(
                        {"timestamp": metric.get("timestamp", time.time()), "value": metric["response_time_ms"]}
                    )
            ...
            for window in time_windows:
                if window:
                    ...
                    time_series.append({"timestamp": window[0].get("timestamp", time.time()), "value": error_rate})

        elif metric_type == "cost":
            # Extract cost data
            for metric in metrics_data:
                if metric.get("cost_usd") is not None:
                    time_series.append({"timestamp": metric.get("timestamp", time.time()), "value": metric["cost_usd"]})
            ...
            for window in time_windows:
                if window:
                    ...
                    time_series.append({"timestamp": window[0].get("timestamp", time.time()), "value": throughput})
            ...
            for metric in metrics_data:
                if metric.get("quality_score") is not None:
                    time_series.append(
                        {"timestamp": metric.get("timestamp", time.time()), "value": metric["quality_score"]}
                    )
            ...
            for window in time_windows:
                if window:
                    total_tokens = sum(m.get("tokens_input", 0) + m.get("tokens_output", 0) for m in window)
                    time_series.append({"timestamp": window[0].get("timestamp", time.time()), "value": total_tokens})
            ...
            for window in time_windows:
                if window:
                    unique_users = len({m.get("user_id") for m in window if m.get("user_id")})
                    time_series.append({"timestamp": window[0].get("timestamp", time.time()), "value": unique_users})
                    ...
                    if data:
                        hourly_stats[hour] = {
                            ...
                            "avg_response_time": statistics.mean(
                                d["response_time"] for d in data if d["response_time"] > 0
                            ),
                ...
                insights.append(
                    {
                        ...
                        "description": "Costs are trending upward with strong correlation",
    ...
    def _create_time_windows(
        self, metrics: list[dict[str, Any]], window_minutes: int = 60
    ...
    def get_trend_summary(self, metric_type: str, days: int = 7) -> dict[str, Any] | None:
        """Get trend summary for a specific metric."""
        if metric_type not in self._trend_history:
            return None

        cutoff_time = time.time() - (days * 24 * 3600)
        recent_data = [point for point in self._trend_history[metric_type] if point["timestamp"] >= cutoff_time]
