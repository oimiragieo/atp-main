...
class TimestampMixin:
    ...
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"), onupdate=datetime.utcnow, nullable=False
    )
...
class Provider(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (Index("idx_providers_type_enabled", "provider_type", "is_enabled"),)
...
class Policy(Base, TimestampMixin, SoftDeleteMixin, TenantMixin):
    ...
    # Metadata
    policy_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
...
class ComplianceViolation(Base, TimestampMixin, TenantMixin):
    ...
    # Remediation
    remediation_steps: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    remediation_notes: Mapped[str | None] = mapped_column(Text, nullable=True)

    # Metadata
    violation_metadata: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
...
class ModelStats(Base, TimestampMixin):
    ...
    avg_tokens_per_request: Mapped[float | None] = mapped_column(Float, nullable=True)

    # Cost statistics
    total_cost_usd: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    avg_cost_per_request: Mapped[float | None] = mapped_column(Float, nullable=True)
