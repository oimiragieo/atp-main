...
class CostOptimizer:
        ...
        try:
            ...
            if self.config.forecasting_enabled:
                forecast = self.cost_forecaster.forecast_cost(
                    horizon_hours=24, model_type=self.config.forecasting_model_type
                )
    ...
    async def _handle_recommendations(self, recommendations: list[dict[str, Any]]) -> None:
        ...
        total_potential_savings = sum(rec.get("potential_savings_usd", 0) for rec in recommendations)

        self._total_savings_identified += total_potential_savings
        self._recommendations_generated += len(recommendations)

        logger.info(
            f"Generated {len(recommendations)} optimization recommendations with ${total_potential_savings:.2f} potential savings"
        )
    ...
    async def _generate_model_substitution_recommendations(
        self, all_pricing: dict[str, dict[str, dict[str, float]]]
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            snapshot.get("token_usage", {}).get("by_model", {})
    ...
    async def _find_cheaper_alternatives(
        self, current_model: str, all_pricing: dict[str, dict[str, dict[str, float]]]
    ) -> list[dict[str, Any]]:
    ...
    async def get_optimization_dashboard(self) -> dict[str, Any]:
        ...
        dashboard = {
            ...
            "system_health": {"budget_manager": "healthy", "cost_forecaster": "healthy", "anomaly_detector": "healthy"},
        }
