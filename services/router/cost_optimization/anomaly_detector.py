...
class CostAnomalyDetector:
    ...
    def detect_anomalies(
        ...
        self._detected_anomalies = [
            a for a in self._detected_anomalies if a["detected_at"] > week_ago
        ]
    ...
    def _detect_cost_outliers(self, data: List[Tuple[float, float, Dict]]) -> List[Dict[str, Any]]:
        ...
        anomalies = []
        threshold = self.threshold_std_dev * std_cost
    ...
    def get_anomaly_summary(
        ...
        # Calculate impact
        total_anomalous_cost = sum(
            anomaly.get("cost_usd", 0) for anomaly in filtered_anomalies
        )
