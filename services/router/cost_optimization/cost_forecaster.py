...
class CostForecaster:
    ...
    def add_data_point(self, cost_usd: float, token_count: int, timestamp: Optional[float] = None) -> None:
    ...
    def forecast_cost(
        ...
        try:
            # Prepare data
            timestamps, costs = zip(*self._cost_history)
    ...
    def _extract_seasonality(
        ...
        detrended_costs = [
            ...
            for h, cost in zip(hours, costs)
        ]
        
        # Group by hour of day
        hourly_patterns = {}
        for h, detrended_cost in zip(hours, detrended_costs):
    ...
    def get_usage_forecast(self, horizon_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            timestamps, token_counts = zip(*self._usage_history)
            current_time = time.time()
            hours_from_now = [(ts - current_time) / 3600 for ts in timestamps]
    ...
    def get_cost_per_token_trend(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cost_dict = dict(self._cost_history)
            usage_dict = dict(self._usage_history)
            ...
            # Analyze trend
            timestamps, cost_per_token_values = zip(*cost_per_token_data)
            current_time = time.time()
            hours_from_now = [(ts - current_time) / 3600 for ts in timestamps]
    ...
    def get_forecast_accuracy(self, actual_costs: List[Tuple[float, float]]) -> Dict[str, Any]:
        ...
        try:
            ...
            errors = [abs(actual - forecast) for actual, forecast in zip(actual_values, forecast_values)]
            relative_errors = [
