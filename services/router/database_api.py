    ...
    try:
        if request.backup_type == "full":
            ...
            if not request.base_backup:
                raise HTTPException(
                    status_code=400,
                    detail="Base backup required for incremental backup"
                )
...
async def restore_database(
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid or corrupted backup file: {request.backup_file}"
            )
        
        # Perform restore
        success = await backup_manager.restore_backup(
            request.backup_file,
            request.target_database if request.target_database else None
        )
        ...
        if db_manager.engine:
            ...
            try:
                async with db_manager.get_session() as session:
                    ...
                    tables = ["requests", "responses", "providers", "models", "policies", 
                             "audit_logs", "compliance_violations", "model_stats"]
