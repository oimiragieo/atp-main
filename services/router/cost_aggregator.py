...
class EnhancedCostAggregator:
    ...
    def enhanced_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "token_usage": {"by_provider": dict(self._tokens_by_provider), "by_model": dict(self._tokens_by_model)},
            "request_counts": {
    ...
    async def get_cost_optimization_insights(self) -> dict[str, Any]:
        ...
        insights = {"top_cost_providers": [], "top_cost_models": [], "cost_efficiency": {}, "recommendations": []}

        try:
            # Top cost providers
            sorted_providers = sorted(self._usd_by_provider.items(), key=lambda x: x[1], reverse=True)
            insights["top_cost_providers"] = [
                {"provider": provider, "cost_usd": cost} for provider, cost in sorted_providers[:5]
            ]

            # Top cost models
            sorted_models = sorted(self._usd_by_model.items(), key=lambda x: x[1], reverse=True)
            insights["top_cost_models"] = [{"model": model, "cost_usd": cost} for model, cost in sorted_models[:5]]

            # Cost efficiency (cost per token)
            for model, cost in self._usd_by_model.items():
                if model in self._tokens_by_model:
                    total_tokens = self._tokens_by_model[model]["input"] + self._tokens_by_model[model]["output"]
                    if total_tokens > 0:
            ...
            if pricing_manager:
                ...
                usage_data = {}
                for provider, _tokens in self._tokens_by_provider.items():
                    usage_data[provider] = {}
