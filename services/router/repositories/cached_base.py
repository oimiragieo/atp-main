...
class CachedBaseRepository(Generic[ModelType]):
    ...
    def __init__(self, model_class: Type[ModelType], cache_prefix: Optional[str] = None):
    ...
    def _make_cache_key(self, key: Union[str, UUID, int], suffix: str = "") -> str:
        """Generate cache key with prefix."""
        base_key = f"{self.cache_prefix}{str(key)}"
        return f"{base_key}:{suffix}" if suffix else base_key
    
    def _make_list_cache_key(self, filters: Optional[Dict[str, Any]] = None, 
    ...
    async def _invalidate_related_caches(self, entity_id: Union[UUID, str, int]) -> None:
    ...
    async def find_one_by(self, use_cache: bool = True, **kwargs) -> Optional[ModelType]:
    ...
    async def clear_cache(self, entity_id: Optional[Union[UUID, str, int]] = None) -> None:
    ...
    async def warm_cache(self, entity_ids: List[Union[UUID, str, int]]) -> int:
