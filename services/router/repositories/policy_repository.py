...
class PolicyRepository(BaseRepository[Policy]):
    ...
    async def get_enabled_policies(self, tenant_id: Optional[str] = None) -> List[Policy]:
    ...
    async def search_policies(
        ...
        async with self.db_manager.get_session() as session:
            ...
            if enabled_only:
                stmt = stmt.where(Policy.is_enabled == True)
    ...
    async def get_policy_statistics(self, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        """Get policy statistics."""
        async with self.db_manager.get_session() as session:
            # Base query
            base_stmt = select(func.count(Policy.id)).where(Policy.is_deleted == False)
            ...
            # Enabled policies
            enabled_stmt = base_stmt.where(Policy.is_enabled == True)
    ...
    async def get_recently_updated_policies(
        ...
        async with self.db_manager.get_session() as session:
            ...
            stmt = (
                select(Policy)
                .where(Policy.is_deleted == False)
                .where(Policy.updated_at >= cutoff_time)
            )
