...
from .pricing_config import PROVIDER_MODEL_MAPPINGS, PricingConfig
from .provider_apis import BasePricingAPI, PricingAPIError, create_pricing_api
...
class PricingMonitor:
    ...
    def _initialize_provider_apis(self) -> None:
        ...
        try:
            ...
            mock_api.set_pricing_data(
                {"test-model": {"input": 0.01, "output": 0.03}, "test-model-2": {"input": 0.005, "output": 0.015}}
            )
    ...
    async def _update_all_pricing(self) -> None:
        ...
        for provider_name, api in self.provider_apis.items():
            if provider_name in PROVIDER_MODEL_MAPPINGS:
                ...
                for model in models:
                    task = asyncio.create_task(self._update_model_pricing(provider_name, model, api))
    ...
    async def _update_model_pricing(self, provider: str, model: str, api: BasePricingAPI) -> bool:
        ...
        try:
            ...
            success = await self.cache.set_pricing(
                ...
                model=model,
                pricing=pricing,
                metadata={"updated_at": time.time(), "api_provider": api.get_provider_name()},
            )
    ...
    async def _check_for_alerts(self, provider: str, model: str, current_pricing: dict[str, float]) -> None:
            ...
            for change in changes:
                ...
                if change_percent >= self.config.significant_change_percent:
                    await self.alert_manager.send_pricing_alert(
                        provider=provider, model=model, change_data=change, severity="high"
                    )
                elif change_percent >= self.config.change_threshold_percent:
                    await self.alert_manager.send_pricing_alert(
                        provider=provider, model=model, change_data=change, severity="medium"
                    )
    ...
    async def get_stale_pricing_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stale_items": [
                {"provider": provider, "model": model, "age_seconds": age_seconds, "age_hours": age_seconds / 3600}
                for provider, model, age_seconds in stale_items
            ],
        }

    async def get_pricing_changes_report(self, since_hours: int = 24) -> dict[str, Any]:
        ...
        significant_changes = [
            c for c in changes if abs(c.get("change_percent", 0)) >= self.config.significant_change_percent
        ]
        moderate_changes = [
            c
            for c in changes
            if self.config.change_threshold_percent
            <= abs(c.get("change_percent", 0))
    ...
    async def validate_pricing_accuracy(
        self, provider: str, model: str, actual_cost: float, tokens_used: int, token_type: str = "input"
    ) -> dict[str, Any]:
