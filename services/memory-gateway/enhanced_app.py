...
from .app import (
    validate_namespace, validate_key, validate_tenant_id, validate_request_size,
    validate_consistency_level, _record_access, _audit_event, STORE
)
...
def _process_data_for_storage(
    ...
    key: str,
    user_id: Optional[str] = None
) -> tuple[Any, list[PIIMatch], Optional[str]]:
...
def put_with_pii_protection(
    ...
    if x_disable_pii_protection:
        ...
        processed_data, pii_matches, audit_id = _process_data_for_storage(
            body.object, x_tenant_id, ns, key, x_user_id
        )
    ...
    if pii_matches:
        audit_data["pii_types"] = list(set(match.pii_type.value for match in pii_matches))
    ...
    if pii_matches:
        ...
        response["pii_types"] = list(set(match.pii_type.value for match in pii_matches))
        if audit_id:
...
def get_pii_audit_trail(
    ...
    try:
        ...
        entries = pii_system.get_audit_trail(
            tenant_id=x_tenant_id,
...
def handle_data_subject_request(
    ...
    try:
        result = pii_system.handle_data_subject_request(
...
def pii_system_health() -> dict[str, Any]:
    ...
    features = {
        ...
        "ml_based_detection": pii_system.detector.config.get("enable_ml_detection", True) and bool(pii_system.detector.ml_models),
        "audit_trail": True,
