...
Examples:
  ...
  python pii_cli.py add-pattern employee_id "EMP\\d{6}" --type custom
    ...
    pattern_parser.add_argument("--type", choices=[t.value for t in PIIType], default="custom", help="PII type")
    ...
    benchmark_parser.add_argument(
        "--methods", nargs="+", choices=["rules", "ml", "both"], default=["both"], help="Detection methods to benchmark"
    )
...
def redact_command(args) -> None:
    ...
    redacted_text, matches, audit_entry = system.process_text(
        args.text, classification, tenant_id=args.tenant, user_id=args.user, return_matches=True
    )
...
def test_file_command(args) -> None:
    ...
    redacted_text, matches, audit_entry = system.process_text(content, classification, return_matches=True)
...
def audit_command(args) -> None:
    ...
    entries = system.get_audit_trail(tenant_id=args.tenant, start_date=start_date, end_date=end_date)
    ...
    if args.format == "json":
        ...
        for entry in entries:
            ...
            pii_types = ", ".join({match["pii_type"] for match in entry.get("pii_matches", [])})[:19]
...
def benchmark_command(args) -> None:
    ...
    def generate_test_text(size: int) -> str:
        ...
        # Add some PII
        pii_samples = ["john.doe@example.com", "(555) 123-4567", "123-45-6789", "John Smith", "4532 1234 5678 9012"]
