...
class GCPCostCollector:
    ...
    async def collect_cost_data(self, start_date: datetime, end_date: datetime) -> list[CostData]:
        ...
        try:
            # Query billing data
            billing_v1.ListProjectBillingInfoRequest(name=f"billingAccounts/{self.billing_account_id}")
...
class CostForecaster:
    ...
    def train_forecast_model(
        self, historical_data: list[CostData], provider: CloudProvider, category: CostCategory
    ) -> None:
        """Train forecasting model for specific provider and category"""
        try:
            # Filter data for specific provider and category
            filtered_data = [d for d in historical_data if d.provider == provider and d.category == category]
    ...
    def generate_forecast(
        self, provider: CloudProvider, category: CostCategory, tenant_id: str, project_id: str, days_ahead: int = 30
    ) -> CostForecast | None:
        ...
        try:
            ...
            for date in future_dates:
                features = [date.dayofweek, date.day, date.month, (date - datetime.now()).days]
            ...
            confidence_interval = (total_forecast - 1.96 * std_error, total_forecast + 1.96 * std_error)
...
class CostOptimizer:
    ...
    async def generate_recommendations(
        self, cost_data: list[CostData], utilization_data: dict[str, float]
        ...
        try:
            ...
            for resource_id, costs in resource_costs.items():
                ...
                if latest_cost.category == CostCategory.COMPUTE:
                    rec = self._analyze_compute_optimization(latest_cost, avg_cost, utilization)
                elif latest_cost.category == CostCategory.STORAGE:
                    rec = self._analyze_storage_optimization(latest_cost, avg_cost, utilization)
                elif latest_cost.category == CostCategory.DATABASE:
                    rec = self._analyze_database_optimization(latest_cost, avg_cost, utilization)
    ...
    def _analyze_compute_optimization(
        self, cost_data: CostData, avg_cost: float, utilization: float
    ...
    def _analyze_database_optimization(
        self, cost_data: CostData, avg_cost: float, utilization: float
    ) -> OptimizationRecommendation | None:
...
class CostTrackingSystem:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        if "gcp" in config:
            self.collectors[CloudProvider.GCP] = GCPCostCollector(
                config["gcp"]["project_id"], config["gcp"]["billing_account_id"]
            )

        if "aws" in config:
            self.collectors[CloudProvider.AWS] = AWSCostCollector(config["aws"].get("region", "us-east-1"))

        if "azure" in config:
            self.collectors[CloudProvider.AZURE] = AzureCostCollector(config["azure"]["subscription_id"])
    ...
    async def collect_all_cost_data(self) -> list[CostData]:
        ...
        tasks = []
        for _provider, collector in self.collectors.items():
    ...
    async def run_cost_analysis(self) -> None:
        ...
        try:
            ...
            self.historical_data = [d for d in self.historical_data if d.timestamp >= cutoff_date]
            ...
            recommendations = await self.optimizer.generate_recommendations(current_data, utilization_data)
    ...
    async def _train_forecasting_models(self) -> None:
        ...
        for provider, category in combinations:
            self.forecaster.train_forecast_model(self.historical_data, provider, category)
    ...
    async def _generate_forecasts(self) -> list[CostForecast]:
        ...
        for provider, category, tenant_id, project_id in combinations:
            forecast = self.forecaster.generate_forecast(provider, category, tenant_id, project_id)
            if forecast:
    ...
    async def _send_alerts(
        self, anomalies: list[CostAnomaly], recommendations: list[OptimizationRecommendation]
    ) -> None:
        ...
        if critical_anomalies or high_value_recs:
            logger.info(
                f"Sending alerts for {len(critical_anomalies)} anomalies and {len(high_value_recs)} recommendations"
            )
...
async def main():
    ...
    config = {
        "gcp": {"project_id": "your-gcp-project", "billing_account_id": "your-billing-account"},
