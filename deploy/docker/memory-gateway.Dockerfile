...
# Multi-stage build for ATP Memory Gateway
...
RUN groupadd -r atp && useradd -r -g atp -u 1000 atp
...
RUN mkdir -p /tmp /var/tmp /app/pii_audit_logs && \
    chown -R atp:atp /app /tmp /var/tmp
